...
const source = await readFile(resolve(process.cwd(), "contracts/TempoCard.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: { "TempoCard.sol": { content: source } },
  settings: { outputSelection: { "*": { "*": ["abi", "evm.bytecode.object"] } } },
};
const output = JSON.parse(solc.compile(JSON.stringify(input)));
const contract = output.contracts?.["TempoCard.sol"]?.TempoCard;
...
console.log(`Deploying TempoCard from ${account.address} on Base...`);
...
console.log("Set NEXT_PUBLIC_TEMPO_CARD_CONTRACT_ADDRESS to this contract address in Vercel.");
